...
module.exports = ({ fabric }) => {
   ...
   */
  fabric.util.object.extend(fabric.Canvas.prototype, {
    setBackgroundImageCenter (image) {
      let canvasWidth = this.getWidth();
      let canvasHeight = this.getHeight();
      fabric.Image.fromURL(image, (oImage) => {
        if (oImage) {
          let oImageWidth = oImage.get('width');
          let oImageHeight = oImage.get('height');
          let top = (canvasHeight - oImageHeight) / 2;
          let left = (canvasWidth - oImageWidth) / 2;
          this.setBackgroundImage(oImage, () => {
            this.renderAll();
          }, {
            top, left, width: oImageWidth, height: oImageHeight
          });
        }
      });
    }
